...
// ================================
// S2 Feature Generator - API 結構
// ================================

type RecomputeFeaturesRequest struct {
	Symbols []string `json:"symbols" validate:"required,min=1,dive,min=3,regexp=^[A-Z0-9]+$"` // 標的：如 ["BTCUSDT", "ETHUSDT"]
	Windows []string `json:"windows" validate:"required,min=1,dive,oneof=1m 5m 1h 4h 1d"`     // 時間窗口：如 ["1h", "4h", "1d"]
	Force   bool     `json:"force,omitempty"`                                                 // 強制重算（忽略快取）
}
...
type Feature struct {
	...
	FeatureType string                 `json:"feature_type"` // ATR/RV/CORRELATION/DEPTH
...
// ATRFeature ATR 特徵
type ATRFeature struct {
	Symbol    string    `json:"symbol"`
	Period    int       `json:"period"`  // 計算週期
	ATR       float64   `json:"atr"`     // ATR 值
	ATRPct    float64   `json:"atr_pct"` // ATR 百分比
	Timestamp int64     `json:"timestamp"`
	CreatedAt time.Time `json:"created_at"`
}

// RealizedVolatilityFeature 已實現波動率特徵
type RealizedVolatilityFeature struct {
	Symbol    string    `json:"symbol"`
	Period    int       `json:"period"` // 計算週期
	RV        float64   `json:"rv"`     // 已實現波動率
	RVPct     float64   `json:"rv_pct"` // RV 百分比
	Timestamp int64     `json:"timestamp"`
	CreatedAt time.Time `json:"created_at"`
}

// CorrelationFeature 相關性特徵
...
type DepthFeature struct {
	Symbol      string    `json:"symbol"`
	BidDepth    float64   `json:"bid_depth"`     // 買盤深度
	AskDepth    float64   `json:"ask_depth"`     // 賣盤深度
	BidAskRatio float64   `json:"bid_ask_ratio"` // 買賣比例
	Spread      float64   `json:"spread"`        // 價差
	SpreadPct   float64   `json:"spread_pct"`    // 價差百分比
	Timestamp   int64     `json:"timestamp"`
	CreatedAt   time.Time `json:"created_at"`
}

// FeatureComputation 特徵計算任務
type FeatureComputation struct {
...
// FeatureSet 特徵集合快照
type FeatureSetSnapshot struct {
