...
type Trade struct {
	...
	Duration   int64     `json:"duration"` // 持倉時間（毫秒）
...
type AutopsyReport struct {
	...
	ReportType  string    `json:"report_type"` // FULL/SUMMARY/QUICK
	Title       string    `json:"title"`
	Summary     string    `json:"summary"` // TL;DR
	Url         string    `json:"url"`
	CreatedAt   time.Time `json:"created_at"`
	CompletedAt time.Time `json:"completed_at"`
}

// AutopsyAnalysis 復盤分析
type AutopsyAnalysis struct {
	AnalysisID   string    `json:"analysis_id"`
	ReportID     string    `json:"report_id"`
	AnalysisType string    `json:"analysis_type"` // TLDR/CHART/COUNTERFACTUAL/PEER/TCA
	Content      string    `json:"content"`       // JSON 字符串
	Score        float64   `json:"score"`         // 分析評分
...
type ChartData struct {
	...
	ReportID  string                 `json:"report_id"`
	ChartType string                 `json:"chart_type"` // PRICE/VOLUME/PNL/DEPTH
	Title     string                 `json:"title"`
	Data      map[string]interface{} `json:"data"`   // 圖表數據
	Config    map[string]interface{} `json:"config"` // 圖表配置
	Url       string                 `json:"url"`
	CreatedAt time.Time              `json:"created_at"`
}

// CounterfactualAnalysis 反事實分析
type CounterfactualAnalysis struct {
	AnalysisID        string    `json:"analysis_id"`
	ReportID          string    `json:"report_id"`
	Scenario          string    `json:"scenario"` // EARLY_EXIT/LATE_EXIT/DIFFERENT_SIZE
	OriginalPnL       float64   `json:"original_pnl"`
	CounterfactualPnL float64   `json:"counterfactual_pnl"`
	Difference        float64   `json:"difference"`
	Probability       float64   `json:"probability"` // 發生概率
...
type PeerComparison struct {
	...
	ReportID       string    `json:"report_id"`
	PeerGroup      string    `json:"peer_group"` // SIMILAR_SYMBOLS/SIMILAR_STRATEGIES/SIMILAR_TIME
	PeerCount      int       `json:"peer_count"`
	AvgPnL         float64   `json:"avg_pnl"`
	MedianPnL      float64   `json:"median_pnl"`
	Percentile     float64   `json:"percentile"`     // 當前交易在同行中的百分位
	Outperformance float64   `json:"outperformance"` // 相對表現
	CreatedAt      time.Time `json:"created_at"`
}

// TCAAnalysis 交易成本分析
type TCAAnalysis struct {
	TCAID           string    `json:"tca_id"`
	ReportID        string    `json:"report_id"`
	ExecutionCost   float64   `json:"execution_cost"`
	MarketImpact    float64   `json:"market_impact"`
	TimingCost      float64   `json:"timing_cost"`
	OpportunityCost float64   `json:"opportunity_cost"`
	TotalCost       float64   `json:"total_cost"`
	CostPct         float64   `json:"cost_pct"`  // 成本佔交易金額百分比
	Benchmark       string    `json:"benchmark"` // 基準比較
	CreatedAt       time.Time `json:"created_at"`
}

// AutopsyTask 復盤任務
type AutopsyTask struct {
	TaskID      string    `json:"task_id"`
	TradeID     string    `json:"trade_id"`
	Status      string    `json:"status"`       // PENDING/RUNNING/COMPLETED/FAILED
	Progress    float64   `json:"progress"`     // 0-100
	ReportTypes []string  `json:"report_types"` // 要生成的報告類型
