...
// ================================
// S3 Strategy Engine - API 結構
// ================================

type DecideRequest struct {
	SignalID  string     `json:"signal_id"`                                           // 唯一信號 ID；用於追蹤/標籤回填
	Symbol    string     `json:"symbol" validate:"required,min=3,regexp=^[A-Z0-9]+$"` // 交易對
	Market    Market     `json:"market" validate:"required,oneof=FUT SPOT"`           // FUT|SPOT
	Features  FeatureSet `json:"features"`                                            // 由 S2 產生的特徵快照
	ConfigRev string     `json:"config_rev" validate:"required"`                      // CURRENT 或整數版本號
	DryRun    bool       `json:"dry_run,omitempty"`                                   // 是否為乾跑（不實際下單）
}
...
type StrategyRule struct {
	...
	RuleType   string    `json:"rule_type"`  // ENTRY/EXIT/RISK
	Conditions string    `json:"conditions"` // JSON 字符串
	Actions    string    `json:"actions"`    // JSON 字符串
...
type StrategyConfig struct {
	...
	Instruments []string               `json:"instruments"` // Symbols
...
type RiskCheck struct {
	...
	SignalID  string    `json:"signal_id"`
	CheckType string    `json:"check_type"` // POSITION_SIZE/MARGIN/CORRELATION
